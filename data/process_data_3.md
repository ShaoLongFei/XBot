...
## 客服解答内容
1. 您好，换一个名称创建就好
2. 您好，很抱歉，空间名称全局唯一，有其他用户创建了这个名称，您无法创建的
3. 您好，很抱歉，您只能换一个名称创建
...
---
## OSS加速应该用主域名，还是创建一个子域名，区别是什么？

**问题分类**：CDN｜配置问题

#
## 详细问题描述
OSS加速应该用主域名，还是创建一个子域名对应OSS存储，区别是什么？，

#
## 客服解答内容
1. 您好，创建没有解析过的子域名，您创建什么域名，后续使用的时候就是用这个域名。
2. 您好，是的，设置七牛云存储是指加速访问对象存储空间的资源。
3. 您好，加速的是您填入的源站域名的相关请求。
4. 加速域名需要填一个未解析过的域名。主域名没解析过的话，可以填源站配置 源站域名 或者 IP地址，能请求到您网站服务器的资源就可以。
...
#
## 客服解答内容
1. 您好，nginx就是我们这边使用的格式了，自有证书参考以下方式上传1、建议您在证书厂商下载证书时，选择 Nginx 服务器类型的证书文件2、进入七牛云管理控制台：[REDACTED_URL] 【上传自有证书】3、使用编辑器打开后续文件，并复制 .crt 或者 .pem 后缀的文件内容到证书内容 （公钥），复制 .key 后缀结尾的内容到 证书私钥 。[图片]如果需要把域名升级为 HTTPS 或者修改证书 的话，需要按照以下步骤绑定证书即可：[REDACTED_URL]
2. 好的，把证书内容贴到工单，公钥和私钥
3. 好的
4. 您好，证书部署好了
...
#
## 详细问题描述
这里勾选 中国大陆，是否表示，海外访问后不扣回源流量，和外网流量，如果不是，应该在那里选择线路，不让海外扣流量

#
## 客服解答内容
您好，选择中国大陆的话域名的节点就分布在国内的，国内用户请求正常计算CDN中国大陆流量和CDN回源流量的，不会计算外网流出流量的海外用户访问的话走的也是CDN中国大陆流量您好，您这边CDN回源流量的话可以参考下这个文档的[REDACTED_URL] CDN 域名访问资源，是不会计算外网流出流量的。只有以下五种情况会计算外网流出流量：使用第三方 CDN 回源七牛对象存储空间的回源请求。使用 qshell 工具的 qdownload 命令下载，但没有指定 cdn_domain 。使用 kodo-browser 工具不使用自有域名下载资源。调用 AWS S3协议 下载空间资源。使用 空间源站域名 下载资源。可以参考下这个文档：[REDACTED_URL]
...
#
## 详细问题描述
create forward job error, signaling code: 10070, msg: app not enable rtmpurl："rtmp://qvs-publish.zhihuischool.cn:2045/zhxt/test_exam_1_1236194_1727070122"streamId："test_exam_1_1236194_1727070122"
